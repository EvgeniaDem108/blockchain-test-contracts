...
## Зачем нужны хэши и подписи в блокчейне
Хэши в блокчейне:
1. Связь блоков - в блокчейнах каждый блок содержит хэш предыдущего блока. Поменял старый блок → сломалась вся цепь хэшей
2. Проверка целостности данных - транзакции хэшируются. Изменилась транзакция - изменился хэш
3. Быстрая проверка есть ли транзакция в блоке - с помощью бинарного дерева поиска Merkle Trees
4. Смысл для dApps:
Без этих механизмов невозможно:
❌ доказать, кто отправил транзакцию
❌ гарантировать, что данные не изменили

Подписи в блокчейне используются для доказательства целостности данных в блоке (чит в бит)
